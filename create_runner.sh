...
usage () {
    echo -e "\nUsage:\n"
    echo -e "  $0 -s [gitlab-server] -n [runner-name] -d [runner-description] -t [runner-token] -i [default-image] -l [runner-tags]\n"
    echo -e "  - Required. gitlab-server is the same server you found your token."
    echo -e "  - Required. runner-name must not contain spaces and should be lower case."
    echo -e "  - Required. runner-description can contain spaces."
    echo -e "  - Required. runner-token is found on yout Gitlab server."
...
# Get arguments
while getopts :n:d:t:i:l:s: options; do
        case $options in
...
if [ -z "${RUNNER_NAME}" ] || [ -z "${RUNNER_DESC}" ] || [ -z ${RUNNER_TOKEN} ] || [ -z ${GITLAB_SERVER} ]; then
...
# Variables
VOLUMES_DIR="${HOME}/docker_volumes"
...
if docker ps | grep ${RUNNER_NAME}; then
    echo -e "\nRunner deployed successfully. Registering with ${GITLAB_SERVER}"
    docker exec ${RUNNER_NAME} gitlab-runner register --non-interactive --url "${GITLAB_SERVER}" --registration-token "${RUNNER_TOKEN}" --description "${RUNNER_DESC}" --executor "docker" --docker-image "${RUNNER_IMAGE}" --docker-volumes "/var/run/docker.sock:/var/run/docker.sock" --tag-list "${RUNNER_TAGS}"
else
